...
class person(db.Model):
    ...
    personsAccount =        db.relationship('userAccounts',back_populates='accountPerson')          #done
...
class companyRole(db.Model):
    ...
    companyRoleUserAccount = db.relationship('userAccount', back_populates='companyRolePerson') #done
    associatedAccountType = db.relationship('accountType', back_populates='companyRoleType')    #missing
    companyRoleCreator = db.relationship('userAccount', back_populates='createdCompanyRoles')   #done
    companyRoleRemover = db.relationship('userAccount', back_populates='removedCompanyRoles')   #done
...
class lease(db.Model):
    ...
    paymentOccurrence = db.relationship('occurrence', back_populates = 'occurrenceOfLeasePayments')           #done
    paymentOccurrenceAfterLease = db.relationship('occurrence', back_populates = 'occurrenceOfPaymentAfterLeases') #done
...
class leaseFee(db.Model):
    ...
    leaseID =           db.Column(db.Integer, db.ForeignKey("userLease.leaseID"), nullable = False)
    ...
    startAfterPeriod =  db.Column(db.Integer, db.ForeignKey("appPeriod.periodID"), nullable = False)
    createdUser =       db.Column(db.Integer, db.ForeignKey("userAccounts.username"), nullable = False)
    createDate =        db.Column(db.DateTime, nullable = False)
